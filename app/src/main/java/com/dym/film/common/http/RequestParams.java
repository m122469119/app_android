...
package com.dym.film.common.http;
...
public class RequestParams implements Serializable
{
    ...
    {
        if (!forceMultipartEntity && bytesParams.isEmpty() && fileParams.isEmpty()) {
    ...
    /*如果是一串字符，可以这样*/
    public static RequestBody createStringBody(String content, String contentType)
    {
        if(contentType==null){
    ...
    private RequestBody createMultipartBody() throws IOException
    {
        ...
        // Add stream params
        for (ConcurrentHashMap.Entry<String, BytesWrapper> entry : bytesParams.entrySet()) {
            BytesWrapper bytesWrapper = entry.getValue();
            if (bytesWrapper.bytes != null) {
                RequestBody streamBody = RequestBody.create(MediaType.parse(bytesWrapper.contentType), bytesWrapper.bytes);
                builder.addFormDataPart(entry.getKey(), bytesWrapper.name, streamBody);
            }
        }
        // Add file params
