package com.dym.film.ui.citylist.data;

import com.dym.film.manager.NetworkManager;
import com.dym.film.ui.citylist.model.CityItem;
import com.dym.film.ui.citylist.widget.ContactItemInterface;
import com.dym.film.ui.citylist.widget.pinyin.PinYin;
...
//				list.add(new CityItem(cityName, PinYin.getPinYin(cityName)));
		...
		public static List<ContactItemInterface> getSampleContactList(ArrayList<NetworkManager.CityRespModel> list1)
		{
			List<ContactItemInterface> list = new ArrayList<ContactItemInterface>();
...
//			{
//				//北京 上海 广州 深圳 西安 天津 重庆 杭州 南京 成都 武汉
//				String cityName = list1.get(i).name;
//				if(cityName.contains("北京")){
//					list.add(new CityItem(cityName, "热门",regionList));
...
//
			...
			for(int i = 0; i < list1.size(); i++)
			{
				...
				if(cityName.contains("重庆")){
					list.add(new CityItem(cityName, "c",list1.get(i).districts));
				}else if(cityName.contains("厦门")){
					list.add(new CityItem(cityName, "x",list1.get(i).districts));
				}else{
					list.add(new CityItem(cityName, PinYin.getPinYin(cityName),list1.get(i).districts));
				}
			}
