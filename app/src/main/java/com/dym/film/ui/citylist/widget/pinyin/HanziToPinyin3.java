...
package com.dym.film.ui.citylist.widget.pinyin;
        ...
        synchronized (HanziToPinyin3.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINESE)) {
                    ...
                    if (DEBUG) {
                        //Logd(TAG, "Self validation. Result: " + doSelfValidation());
                    }
                    sInstance = new HanziToPinyin3(true);
                    return sInstance;
                }
            }
            //Logw(TAG, "There is no Chinese collator, HanziToPinyin is disabled");
